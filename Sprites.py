...
class SpriteSheet:  # This deals with the animation for the player
    def __init__(self, file):
        self.sheet = pygame.image.load(file).convert()  # Load image in
...
class Player(pygame.sprite.Sprite):  # pygame.sprite.Sprite a class in the pygame module to make it easy to make sprites
    def __init__(self, game, x, y):  # by passing in game we can access all variables in class Game
        ...
        self.x_change = 0  # Temp variables that will store the change in movement during one loop
        self.y_change = 0

        self.facing = 'down'  # When we start animating we need to know which direction the player is facing
    ...
    def update(self):
        self.movement()  # calling movement
        self.animate()
        self.collide_enemy()
    ...
    def movement(self):
        ...
        if keys[pygame.K_LEFT]:  # Check if left arrow key is pressed
            ...
            self.x_change -= PLAYER_SPEED  # if we want to move left we take away from x_change
            self.facing = 'left'  # Player sprite turns left when animated
    ...
    def collide_blocks(self, direction):
        if direction == "x":
            ...
            if hits:  # if there is a collision it will be set to True
    ...
    def animate(self):  # taking from the sprite sheet the cords, then also taking the width of the sprite 32x32
        if self.facing == "down":
...
class Block(pygame.sprite.Sprite):
    def __init__(self, game, x, y):
        ...
        self.groups = self.game.all_sprites, self.game.blocks  # Another group in def new(self)
        pygame.sprite.Sprite.__init__(self, self.groups)  # This is calling the init method for the inherited class
        self.x = x * TILESIZE
        self.y = y * TILESIZE
        self.width = TILESIZE
        self.height = TILESIZE
...
class Ground(pygame.sprite.Sprite):
    def __init__(self, game, x, y):
        ...
        self.image = self.game.terrain_spritesheet.get_sprite(64, 352, self.width, self.width)
        self.rect = self.image.get_rect()
        self.rect.x = self.x
        self.rect.y = self.y
